...
## Note that time series shorter than 2004 - 2010 will give error in dataframe; set write.output <- F to avoid that error
## Version m uses start.yr if larger than select.yr; also, the csv output table contains the functional group from the ID file
##---------------------------------------------------------------------------------------------
...
#-----------------------------------------O_Stocks_Catch_8_Med.csv
# Required settings, File names
#-----------------------------------------
catch_file  <-  "O_ICES_Catch_13.csv" #"O_Stocks_Catch_8_Med.csv"    # name of file containing "stock", "yr", "ct", and optional "bt"
id_file     <-  "O_ICES_ID_17.csv"    #"O_Stocks_ID_8_Med.csv"  # name of file containing stock-specific info and settings for the analysis
...
# stocks <- "her-3a22"  # c("bll-2232","cod-2224","dab-2232","fle-2223","fle-2425","ple-2123","ple-2432","spr-2232","tur-2232")     #c("cod-2224") #c("tur-kask")   #c("cod-scow") # c("MULLBAR_BA") #c("CORYHIP_SA")  #   c("Tur_BS")  #  #   "sal-2231"    #c("sol-iris") # c("cod-2224")   
...
select.yr    <- 2014 # option to display F, B, F/Fmsy and B/Bmsy for a certain year; default NA
...
if(is.na(stocks[1])==TRUE){
  ...
  # stocks         <- sort(as.character(cinfo$Stock)) # Analyze stocks in alphabetic order
   stocks         <- as.character(cinfo$Stock[cinfo$Subregion=="Greater North Sea"]) # Analyze stocks in Region
}
